...
namespace yolo11 {
    ...
    Yolo11Detector::Yolo11Detector() = default;

    Yolo11Detector::~Yolo11Detector() {
        release();
    }

    bool Yolo11Detector::init(const DetectorConfig& config) {
        if (initialized_) {
            release();
        }
        ...
        std::cout << "Yolo11Detector initialized successfully." << std::endl;
    ...
    bool Yolo11Detector::loadEngine(const std::string& engine_path) {
    ...
    bool Yolo11Detector::prepareBuffers() {
        auto* engine = reinterpret_cast<nvinfer1::ICudaEngine*>(engine_);
        if (!engine) {
        ...
        if (engine->getNbIOTensors() != 2) {
            std::cerr << "Detection model should have 1 input and 1 output." << std::endl;
            return false;
        }
        ...
        CUDA_CHECK(cudaMalloc(
            ...
            kBatchSize * kDetOutputSize * sizeof(float)
        ));

        if (!config_.use_gpu_postprocess) {
            output_host_ = new float[kBatchSize * kDetOutputSize];
        }
        else {
            if (kBatchSize > 1) {
    ...
    std::vector<Detection> Yolo11Detector::infer(const cv::Mat& image) {
    ...
    std::vector<std::vector<Detection>>
        Yolo11Detector::inferBatch(const std::vector<cv::Mat>& images) {
        std::vector<std::vector<Detection>> results;

        if (!initialized_) {
            std::cerr << "Detector is not initialized." << std::endl;
            return results;
        }
