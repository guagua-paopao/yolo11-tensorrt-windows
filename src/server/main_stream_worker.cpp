...
namespace {
    ...
    [[noreturn]] void terminateHandler() noexcept {
        std::cerr << "Fatal stream worker error: std::terminate called." << std::endl;
        std::cerr.flush();
#ifdef _WIN32
    ...
    BOOL WINAPI consoleCtrlHandler(DWORD ctrl_type) {
        ...
        case CTRL_SHUTDOWN_EVENT:
            std::cout << "Stop signal received. Stopping YOLO11 stream worker..." << std::endl;
            requestStop();
            return TRUE;
        default:
            return FALSE;
    ...
    void printUsage() {
        std::cout << "Usage:\n"
            << "  yolo11_stream_worker.exe <config.yaml> [--worker-num N] [--consumer-name stream_worker_1] [--gpu-id 0]\n\n"
            << "Examples:\n"
            << "  yolo11_stream_worker.exe D:/tensorrtx/yolo11/config/worker_stream.yaml --consumer-name stream_worker_1\n"
            << "  yolo11_stream_worker.exe D:/tensorrtx/yolo11/config/worker_stream.yaml --worker-num 1\n";
    }
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        app_config.stream.enabled = true;
        app_config.model.type = "detect";

        std::string logger_error;
        if (!yolo11_server::initializeLogger(app_config, "stream_worker", logger_error)) {
        ...
        spdlog::info("YOLO11 stream worker process starting.");
        ...
        spdlog::info("Stream snapshot_dir={}, default_source_type={}", app_config.stream.snapshot_dir, app_config.stream.default_source_type);

        std::vector<std::unique_ptr<yolo11_server::StreamInferenceWorker>> workers;
        if (single_consumer_mode) {
            spdlog::info("Stream worker mode: single consumer, consumer_name={}", consumer_name_override);
            auto worker = std::make_unique<yolo11_server::StreamInferenceWorker>(1, app_config, consumer_name_override);
            if (!worker->start()) {
                spdlog::error("Failed to start stream worker: {}", consumer_name_override);
#ifdef _WIN32
                WSACleanup();
#endif
                return -1;
            ...
            spdlog::info("Stream worker mode: internal worker pool, worker_num={}", app_config.worker.worker_num);
            for (int i = 1; i <= app_config.worker.worker_num; ++i) {
                ...
                auto worker = std::make_unique<yolo11_server::StreamInferenceWorker>(i, app_config, consumer_name);
                if (!worker->start()) {
                    spdlog::error("Failed to start stream worker: {}", consumer_name);
#ifdef _WIN32
                    WSACleanup();
#endif
                    return -1;
        ...
        waitForStop();
        spdlog::info("Stopping stream workers...");
        for (auto& worker : workers) {
            if (worker) {
                worker->stop();
            }
        }
        spdlog::info("YOLO11 stream worker process stopped.");
    }
    catch (const std::exception& e) {
        spdlog::error("Fatal stream worker error: {}", e.what());
        exit_code = -1;
    }
    catch (...) {
        spdlog::error("Fatal stream worker error: unknown exception");
        exit_code = -1;
    }
