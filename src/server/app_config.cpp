...
namespace yolo11_server {
    ...
    namespace {
        ...
        T readOrDefault(const YAML::Node& node, const std::string& key, const T& default_value) {
            ...
            try {
                ...
                std::cerr << "Config parse warning: key=" << key
                    << ", reason=" << e.what()
    ...
    AppConfig AppConfig::loadFromYaml(const std::string& yaml_path) {
        ...
        config.model.use_gpu_postprocess =
            readOrDefault<bool>(model, "use_gpu_postprocess", config.model.use_gpu_postprocess);

        auto output = root["output"];
        config.output.save_result_image =
            readOrDefault<bool>(output, "save_result_image", config.output.save_result_image);
